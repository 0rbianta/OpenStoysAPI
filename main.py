...
def main():
    stoys = Stoys()
    user_data = stoys.login("USERNAME_HERE", "PASSWORD_HERE")
    if user_data is None:
        print("Problem with connection occurred. Canceling process...")
        return

    auth = user_data["result"]["token"]
    student_id = str(user_data["result"]["student_id"])
    user_id = str(user_data["result"]["user_id"])
    sinif_id = str(user_data["result"]["sinif_id"])
    abl = stoys.get_absence_list(auth, student_id)
    ac = stoys.get_academic_calendar(auth, student_id, user_id)

    print(ac)

    ad = stoys.get_academic_details(auth, student_id, user_id)
    a = stoys.get_announcements(auth, user_id)
    try:
        first_announcement_id = a["result"][0]["ref_id"]
        first_announcement_type = a["result"][0]["tip"]
        annd = stoys.get_announcement_details(auth, first_announcement_id, first_announcement_type) 
    except KeyError:
        pass
    except IndexError:
        pass

    c = stoys.get_conversations(auth, user_id)

    try:
        first_conversation_id = c["result"][0]["_id"]
        cd = stoys.get_conversations_details(auth, first_conversation_id)
    except KeyError:
        pass
    except IndexError:
        pass

    # Warning: get_daily_state returns html. Not json...
    ds = stoys.get_daily_state(auth, student_id, stoys.Date.dateFormat2.create_stoys_adapted_date("10", "9", "2021"))

    ed = stoys.get_etude_calendar(auth, student_id)

    ec = stoys.get_exam_calendar(auth, student_id, stoys.Date.dateFormat11.create_stoys_adapted_date("10", "9", "2021"),
                                 stoys.Date.dateFormat11.create_stoys_adapted_date("20", "9", "2021"))

    et = stoys.get_exam_types(auth, student_id)


    g = stoys.get_gallery(auth, student_id)

    t = stoys.get_teacher(auth, sinif_id)

    g = stoys.get_guardian(auth)

    g = stoys.get_guidance(auth, student_id, user_id)


    h = stoys.get_homeworks(auth, student_id)

    try:
        first_homework_id = h["result"][0]["id"]
        hd = stoys.get_homework_details(auth, student_id, first_homework_id)
    except IndexError:
        print("No homeworks assigned")
    except Exception:
        pass

    oe = stoys.get_open_exam(auth, student_id)

    sv = stoys.get_schedule_view(auth, student_id)

    si = stoys.get_student_information(auth, student_id)
